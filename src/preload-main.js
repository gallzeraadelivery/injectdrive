...
(function(){
  ...
  try {
    ...
    } else if (f.preset==='desktop') {
      var _uaPlat = (f.platform||'Win32').indexOf('Mac')>=0 ? 'macOS' : (f.platform||'').indexOf('Linux')>=0 ? 'Linux' : 'Windows';
      Object.defineProperty(navigator,'userAgentData',{value:{brands:[{brand:'Chromium',version:'131'},{brand:'Google Chrome',version:'131'},{brand:'Not_A Brand',version:'24'}],mobile:false,platform:_uaPlat,getHighEntropyValues:h=>Promise.resolve({})},configurable:true,writable:false});
    }
    if (f.timezone) {
...
(function injectFingerprint() {
  try {
    ...
    } else if (fp.preset === 'desktop') {
      try {
        const uaPlat = (fp.platform || 'Win32').includes('Mac') ? 'macOS' : (fp.platform || '').includes('Linux') ? 'Linux' : 'Windows';
        const uaData = {
          brands: [{ brand: 'Chromium', version: '131' }, { brand: 'Google Chrome', version: '131' }, { brand: 'Not_A Brand', version: '24' }],
          mobile: false,
          platform: uaPlat,
          getHighEntropyValues: (hints) => Promise.resolve({
            ...(hints.includes('platform') && { platform: uaPlat }),
            ...(hints.includes('platformVersion') && { platformVersion: uaPlat === 'Windows' ? '10.0.0' : uaPlat === 'macOS' ? '14.0.0' : '6.0.0' }),
...
function getDeviceIdsForPreset() {
  try {
    ...
    const maxTouchPoints = typeof fp.maxTouchPoints === 'number' ? fp.maxTouchPoints : (isMobile ? 5 : 0);
    return { video: v, videoFront: vFront, audio: a, group: g, maxTouchPoints, ...labels };
  } catch {
    return {
...
(() => {
  ...
  // Touch: mobile = 5, desktop = 0
  try { Object.defineProperty(navigator, 'maxTouchPoints', { value: ${JSON.stringify(typeof ids.maxTouchPoints === 'number' ? ids.maxTouchPoints : 5)}, configurable: true }); } catch {}
  ...
  function stopObsCamera() {
    if (obsStream) {
      obsStream.getTracks().forEach(t => { try { t.stop(); } catch(e) {} });
      obsStream = null;
    }
    if (obsVideoEl) {
      try {
        obsVideoEl.srcObject = null;
        if (obsVideoEl.parentNode) obsVideoEl.parentNode.removeChild(obsVideoEl);
      } catch(e) {}
      obsVideoEl = null;
    }
  }

  function attachLiveVideo(vid, stream) {
    vid.autoplay = true;
    vid.muted = true;
  ...
  async function openObsStream(deviceId) {
    const attempts = [
      { video: { deviceId: { exact: deviceId }, width: { ideal: BASE_W }, height: { ideal: BASE_H } }, audio: false },
    ...
    for (const constraints of attempts) {
      try {
        return await ORIG.gum(constraints);
      } catch (e) {
        lastErr = e;
    ...
    throw lastErr || new Error('OBS Virtual Camera recusou as configurações (OverconstrainedError).');
  }

  async function useObsCamera(deviceId) {
    stopObsCamera();
    if (!ORIG.gum) {
      return { ok: false, error: 'getUserMedia original indisponível nesta página.' };
    }
    let targetId = null;
    let targetLabel = '';
    if (ORIG.enum) {
      try {
        const devices = await ORIG.enum();
        const obs = findObsDevice(devices);
        targetId = obs?.deviceId || null;
        targetLabel = obs?.label || '';
      } catch (e) {
        return { ok: false, error: 'Erro ao listar câmeras: ' + (e.message || e) };
      }
    }
    if (!targetId && deviceId) targetId = deviceId;
    if (!targetId) {
      return { ok: false, error: 'OBS Virtual Camera não encontrada. No OBS: Controles → Iniciar câmera virtual.' };
    }
    try {
      obsStream = await openObsStream(targetId);
      const trackLabel = obsStream.getVideoTracks()[0]?.label || targetLabel || '';
      if (!isObsVirtualCamera(trackLabel)) {
        stopObsCamera();
        return {
          ok: false,
          error: 'Conectou na câmera errada (' + (trackLabel || 'desconhecida') + '). Inicie a Câmera Virtual no OBS Studio e tente de novo.',
        };
      }
      const vid = document.createElement('video');
      attachLiveVideo(vid, obsStream);
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('Timeout ao receber frames do OBS')), 12000);
        const done = () => {
          clearTimeout(timeout);
          if (vid.videoWidth > 0 && vid.videoHeight > 0) resolve();
          else reject(new Error('OBS conectou mas não enviou frames válidos'));
        };
        vid.onloadedmetadata = () => {
          const p = vid.play();
          if (p && p.then) p.then(done).catch(reject);
          else done();
        };
        vid.onerror = () => reject(new Error('Erro ao reproduzir stream do OBS'));
      });
      obsVideoEl = vid;
      media = vid;
      const label = obsStream.getVideoTracks()[0]?.label || 'OBS Virtual Camera';
      console.log('[Fakecam/OBS] Conectado:', label, vid.videoWidth + 'x' + vid.videoHeight);
      forceDrawOnce();
      return { ok: true, label, deviceId: targetId, width: vid.videoWidth, height: vid.videoHeight };
    } catch (e) {
      stopObsCamera();
      console.error('[Fakecam/OBS] Erro:', e);
      const name = e && e.name ? e.name : '';
      if (name === 'OverconstrainedError' || name === 'ConstraintNotSatisfiedError') {
        return { ok: false, error: 'OBS encontrado mas não aceitou o stream. Pare e reinicie a Câmera Virtual no OBS, depois clique OBS de novo.' };
      }
      if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
        return { ok: false, error: 'OBS Virtual Camera não disponível. Inicie a Câmera Virtual no OBS Studio.' };
      }
  ...
  window.addEventListener('message', (ev) => {
    ...
    if (d.type === 'SET_OBS_CAMERA') {
      useObsCamera(d.deviceId).then((result) => {
        window.postMessage({ __FAKECAM__: true, type: 'OBS_CAMERA_RESULT', ...(result || { ok: false }) }, '*');
      });
    }
    if (d.type === 'STOP_OBS_CAMERA') stopObsCamera();
...
ipcRenderer.on('fakecam:useObsCamera', (_e, payload) => {
  const deviceId = payload?.deviceId || null;
  window.postMessage({ __FAKECAM__: true, type: 'SET_OBS_CAMERA', deviceId }, '*');
});

window.addEventListener('message', (event) => {
  if (event.data && event.data.__FAKECAM__ === true && event.data.type === 'OBS_CAMERA_RESULT') {
    try {
      ipcRenderer.send('fakecam:obsCameraResult', event.data);
    } catch (e) {
      console.warn('[Preload] Erro ao enviar resultado OBS:', e);
    }
  }
