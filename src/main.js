...
const { generateFingerprint, mergeFingerprint, ensureFingerprintFields, applyUsaLocale, applyGeoToFingerprint } = require('./fingerprint');
...
let obsCameraActive = false;
let currentObsDeviceId = null;
let pendingObsCameraResult = null;
...
function normalizeProfile(p) {
  ...
  let fp = hasCompleteFp
    ? { ...rawFp, preset }
    : (rawFp ? ensureFingerprintFields({ ...rawFp, preset }, preset) : getDefaultFingerprint());

  // Sem proxy: locale sempre USA. Com proxy: mantém o que o sync do geo gravou.
  const hasProxy = !!(lastProxy.host && lastProxy.port);
  const beforeLocale = JSON.stringify({ language: fp.language, languages: fp.languages, timezone: fp.timezone });
  if (!hasProxy) {
    fp = applyUsaLocale(fp);
  } else if (!fp.language || !fp.timezone || !Array.isArray(fp.languages)) {
    fp = applyUsaLocale(fp);
  }
  const afterLocale = JSON.stringify({ language: fp.language, languages: fp.languages, timezone: fp.timezone });
  const localeChanged = beforeLocale !== afterLocale;

  return {
    ...p,
    url: p.url || TCC_URL,
    fingerprint: fp,
    __fingerprintWasCompleted: !hasCompleteFp || localeChanged,
  };
}
...
async function recreateMain(profileId) {
  ...
  // Locale: com proxy → geo do IP; sem proxy → USA
  try {
    await syncFingerprintWithProxyGeo();
  } catch (_) {
    // Continua mesmo se o geo falhar
  }
  ...
  const logicalW = isMobilePreset && ANDROID_EMULATE_VIEWPORT
    ...
    : Math.min(typeof fp.width === 'number' ? fp.width : 1280, 1600);
  const logicalH = isMobilePreset && ANDROID_EMULATE_VIEWPORT
    ? Math.floor(screenH / pixelRatio)
    : Math.min(typeof fp.height === 'number' ? fp.height : 720, 1000);
    ...
    try {
      ...
      if (storedFakecamScript) {
        frame.executeJavaScript(storedFakecamScript).then(() => {
          if (obsCameraActive) {
            frame.executeJavaScript(
              `window.postMessage({ __FAKECAM__: true, type: 'SET_OBS_CAMERA', deviceId: ${JSON.stringify(currentObsDeviceId)} }, '*');`
            ).catch(() => {});
          } else if (currentFakecamVideoDataUrl) {
            frame.executeJavaScript(
...
/* ================ GEO: proxy → locale do IP; sem proxy → USA ================= */
...
async function syncFingerprintWithProxyGeo() {
  ...
  let updated;
  if (!hasProxy) {
    updated = applyUsaLocale(p.fingerprint);
    console.log('[Geo] Sem proxy → locale USA:', updated.timezone, updated.language);
  } else {
    const geo = await fetchGeoFromProxy();
    if (geo) {
      updated = applyGeoToFingerprint(p.fingerprint, geo);
      console.log('[Geo] Proxy →', geo.countryCode, geo.timezone, updated.language);
    } else {
      // Proxy ativo mas geo falhou: mantém fingerprint atual (não força USA para não mismatchar)
      console.warn('[Geo] Proxy ativo mas geo indisponível — mantendo locale atual');
      return;
    }
  }

  if (
    updated.language !== p.fingerprint.language ||
    updated.timezone !== p.fingerprint.timezone ||
    JSON.stringify(updated.languages) !== JSON.stringify(p.fingerprint.languages)
  ) {
    upsertProfile({ ...p, fingerprint: updated });
    currentFingerprintForPreload = updated;
  }
}
...
ipcMain.handle('proxy:change', async (_event, cfg) => {
  ...
  // Recarrega para aplicar Accept-Language + timezone do novo IP (ou USA se removeu proxy)
  if (currentProfileId) {
    await recreateMain(currentProfileId);
  }
});
...
function broadcastObsCameraToIframes(deviceId) {
  if (!mainWin || mainWin.isDestroyed()) return;
  try {
    const wc = mainWin.webContents;
    const code = `window.postMessage({ __FAKECAM__: true, type: 'SET_OBS_CAMERA', deviceId: ${JSON.stringify(deviceId || null)} }, '*');`;
    if (wc.mainFrame && wc.mainFrame.frames) {
      wc.mainFrame.frames.forEach((frame) => {
        if (frame !== wc.mainFrame) {
          frame.executeJavaScript(code).catch(() => {});
    ...
    console.warn('[Fakecam/OBS] Erro ao enviar OBS para iframes:', e);
  }
}

ipcMain.handle('fakecam:listVideoDevices', async () => {
  const win = toolboxWin && !toolboxWin.isDestroyed() ? toolboxWin : mainWin;
  if (!win || win.isDestroyed()) {
    return { ok: false, devices: [], error: 'Nenhuma janela disponível para listar câmeras.' };
  }
  try {
    return await win.webContents.executeJavaScript(`
      (async () => {
        ...
        try {
          ...
          const needsLabels = devices.length > 0 && devices.every(d => !d.label);
          if (needsLabels) {
            let temp;
            try {
              const obsGuess = findObsDevice(all);
              if (obsGuess?.deviceId) {
                temp = await navigator.mediaDevices.getUserMedia({
                  video: { deviceId: { exact: obsGuess.deviceId } },
                  audio: false,
                });
              } else {
              ...
              return { ok: false, devices: [], error: 'Permissão de câmera negada: ' + (e.message || e) };
            }
            temp.getTracks().forEach(t => { try { t.stop(); } catch (_) {} });
            all = await navigator.mediaDevices.enumerateDevices();
            devices = all
              .filter(d => d.kind === 'videoinput')
              .map(d => ({ deviceId: d.deviceId, label: d.label || 'Câmera sem nome' }));
          } else {
            devices = devices.map(d => ({
              deviceId: d.deviceId,
              label: d.label || 'Câmera sem nome',
            }));
          }
          const obs = findObsDevice(all);
          return { ok: true, devices, obsDeviceId: obs?.deviceId || null, obsLabel: obs?.label || null };
        } catch (e) {
...
ipcMain.handle('fakecam:useObsCamera', async (_event, deviceId) => {
  if (!mainWin || mainWin.isDestroyed()) {
    return { ok: false, error: 'Janela principal não aberta. Faça login e abra o navegador primeiro.' };
  }
  return new Promise((resolve) => {
    pendingObsCameraResult = resolve;
    mainWin.webContents.send('fakecam:useObsCamera', { deviceId: deviceId || null });
    setTimeout(() => {
      if (pendingObsCameraResult === resolve) {
        pendingObsCameraResult = null;
        resolve({ ok: false, error: 'Timeout ao conectar OBS Virtual Camera (15s).' });
      }
    }, 15000);
  });
});

ipcMain.on('fakecam:obsCameraResult', (_event, result) => {
  const label = result?.label || '';
  const looksObs = /obs virtual camera|obs-camera|^obs virtual/i.test(String(label).toLowerCase())
    || (/\bobs\b/i.test(label) && /virtual|camera/i.test(label));
  const blocked = /droidcam|manycam|snap camera/i.test(String(label).toLowerCase());
  if (result?.ok && (!looksObs || blocked)) {
    result = {
      ok: false,
      error: 'Conectou na câmera errada (' + (label || 'desconhecida') + '). Inicie a Câmera Virtual no OBS.',
    };
  }
  if (!result?.ok) {
    obsCameraActive = false;
    ...
    obsCameraActive = true;
    currentObsDeviceId = result.deviceId || null;
    currentFakecamImageDataUrl = '';
    currentFakecamVideoDataUrl = '';
    broadcastObsCameraToIframes(currentObsDeviceId);
  }
  if (pendingObsCameraResult) {
    const resolve = pendingObsCameraResult;
    pendingObsCameraResult = null;
    resolve(result || { ok: false, error: 'Resposta inválida do OBS.' });
  }
});
...
/* ================ COOKIES (formato AdsPower) ================= */
function mapSameSiteToAds(sameSite) {
...
function mapSameSiteFromAds(sameSite) {
  const s = String(sameSite || '').toLowerCase();
  if (s === 'lax') return 'lax';
  if (s === 'strict') return 'strict';
  if (s === 'no_restriction' || s === 'none') return 'no_restriction';
...
function toAdsPowerCookie(c) {
  ...
  return {
    ...
    session: isSession,
    expires,
    sameSite: mapSameSiteToAds(c.sameSite),
  };
}

ipcMain.handle('cookies:export', async () => {
  if (!currentPartition) {
    return { ok: false, error: 'Nenhum perfil ativo.' };
  }
  const p = getProfileById(currentProfileId);
  const profileName = (p?.name || currentProfileId || 'perfil').replace(/[^\w\-]+/g, '_');
  const sess = session.fromPartition(currentPartition);
  const list = await sess.cookies.get({});
  const adsCookies = list.map(toAdsPowerCookie);

  const result = await dialog.showSaveDialog(toolboxWin || mainWin, {
    title: 'Baixar cookies (AdsPower)',
    defaultPath: `adspower-cookies-${profileName}.json`,
  ...
  fs.writeFileSync(result.filePath, JSON.stringify(adsCookies), 'utf8');
  return { ok: true, count: adsCookies.length, file: result.filePath };
});
...
ipcMain.handle('cookies:import', async () => {
  ...
  const open = await dialog.showOpenDialog(toolboxWin || mainWin, {
    title: 'Importar cookies (AdsPower)',
    filters: [{ name: 'JSON', extensions: ['json'] }],
    properties: ['openFile'],
  });
  ...
  try {
    ...
    return { ok: false, error: 'Arquivo JSON inválido: ' + (e.message || e) };
  }

  // Aceita array direto (AdsPower) ou { cookies: [...] }
  const cookies = Array.isArray(parsed)
    ? parsed
    : (Array.isArray(parsed?.cookies) ? parsed.cookies : null);
  if (!cookies) {
    return { ok: false, error: 'Formato inválido. Esperado array JSON no padrão AdsPower.' };
  }

  const sess = session.fromPartition(currentPartition);

  // Substitui: limpa cookies atuais e aplica os do arquivo
  try {
    await sess.clearStorageData({ storages: ['cookies'] });
  } catch (e) {
    console.warn('[Cookies] clearStorageData:', e?.message || e);
  }
  ...
  for (const c of cookies) {
    if (!c || !c.name || c.value == null || !c.domain) {
      failCount += 1;
      continue;
    }
    const url = cookieUrlFromAds(c);
    if (!url) {
      failCount += 1;
      continue;
    }
    ...
    const details = {
      ...
      secure: !!c.secure,
      httpOnly: !!c.httpOnly,
      sameSite: mapSameSiteFromAds(c.sameSite),
    };

    // sameSite none exige secure no Chromium
    if (details.sameSite === 'no_restriction' && !details.secure) {
      details.secure = true;
      details.url = cookieUrlFromAds({ ...c, secure: true });
    }
    ...
    try {
      ...
      failCount += 1;
      if (errors.length < 5) {
        errors.push(`${c.name}@${c.domain}: ${e.message || e}`);
      }
    }
  }
  ...
  // Recarrega a página para usar os cookies novos
  if (mainWin && !mainWin.isDestroyed()) {
    try {
      mainWin.webContents.reload();
    } catch (_) {}
